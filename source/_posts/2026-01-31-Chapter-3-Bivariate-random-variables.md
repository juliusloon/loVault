...
$$\Longrightarrow f_X(x) = \int ^{+ \infty}_{- \infty}f(x, y)dy$$$$\Longrightarrow f_Y(y) = \int ^{+ \infty}_{- \infty} f(x, y)dx$$﻿
...
> therefore  
> $$F_{X|Y}(x|y) =\lim _{\epsilon \rightarrow 0^+}\frac{P(X\le x, y<Y<y+ \epsilon)}{P(y<Y<y+\epsilon)} = \lim _{\epsilon \rightarrow 0^+}\frac{F(x, y+ \epsilon)-F(x,y)}{F_Y(y+\epsilon)-F_Y(y)} = \lim _{\epsilon \rightarrow 0^+}\frac{\frac{F(x, y+ \epsilon)-F(x,y)}{\epsilon}}{\frac{F_Y(y+\epsilon)-F_Y(y)}{\epsilon}} =\frac{\frac{\partial F(x,y)}{\partial y}}{F'_Y(y)}$$$$= \frac{\frac{\partial(\int ^{y}_{- \infty}\int^x_{- \infty}f(u, v)dudv)}{\partial y}}{f_Y(y)}=\frac{\int ^{x}_{- \infty}f(u, y)du}{f_Y(y)} = \int ^{x}_{- \infty}\frac{f(u, y)}{f_Y(y)}du$$
...
$$(X, Y) \sim N(\mu_1, \mu_2; \sigma_1^2, \sigma_2^2; \rho) \Rightarrow aX+bY \sim N(a\mu_1+b\mu_2, (a\sigma_1)^2+(b\sigma_2)^2+2ab\rho\sigma_1\sigma_2)$$﻿$$\Longrightarrow X\sim N(\mu_1, \sigma_1^2), Y\sim N(\mu_2, \sigma_2^2),X, Y\text{ are independent, then }aX+bY \sim N(a\mu_1+b\mu_2, (a\sigma_1)^2+(b\sigma_2)^2)$$﻿
...
> Derivation: $$F_Z(z) = P\{aX+bY\le z\}=\int^{+\infty}_{-\infty}dy\int^{+\infty}_{-\infty}f(x,y)dx \overset{ax+by\equiv u}=\int^{+\infty}_{-\infty}d(\frac{u-ax}{b})\int^{+\infty}_{-\infty}f(x,\frac{u-ax}{b})dx$$$$=\frac{1}{|b|}\int^{z}_{-\infty}du\int^{+\infty}_{-\infty}f(x,\frac{u-ax}{b})dx$$
